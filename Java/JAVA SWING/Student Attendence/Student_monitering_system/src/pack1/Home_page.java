...
//	private JLayeredPane layeredPane;
	...
	public Home_page(String idd) {
		...
		obj = list.get(f);
		//
		x =Float.parseFloat(obj.getOclass());
		y=Float.parseFloat(obj.getAbsent());
		per = (x-y)/x;
		nper =per*100;
		ave +=nper;
		coun++;
		p=Float.toString(nper);
		String pu =p+" %";
		...
		textF.setBounds(652, 177, 56, 22);
		HOME.add(textF);
		textF.setColumns(10);
		
		float nc = ave/coun;
		String v =Float.toString(nc);
		textF.setText(v);
		if(nc<=25)
		{
